...
- ⬜ **SIRADA — Faz 6a — Görsel/UX Yenileme:** Kullanıcı canlı siteyi inceleyip ekran görüntüleriyle somut geri bildirim verdi (site işlevsel ama görsel olarak düz, syntax highlighting yok, butonlar ayırt edilemiyor, Öğren sayfası ünitelere gruplanmamış, alıştırmalar akışı SQLBolt tarzı sıralı değil, mülakat kartları seviyeye göre renklenmemiş). Detaylı plan (kesinleşen kararlar: krem+mavi renk paleti, Öğren'de aynı sayfada aç/kapa ünite kutuları, alıştırmalarda yumuşak sıralı yönlendirme, "Sınav" → "Test Sınavı" yeniden adlandırma) ayrı bir plan dosyasında: `C:\Users\yunus\.claude\plans\ren-k-sm-kutulu-bir-partitioned-hare.md`. Context bütçesi nedeniyle uygulama sonraki bir oturumda yapılacak — o plan dosyasındaki "Devam Promptu" ile başlanmalı.
  *Doğrulama: plan dosyasındaki "Doğrulama" bölümüne bakın (npm run check + build + tarayıcıda açık/karanlık/mobil uçtan uca test).*
- ⬜ **Faz 6b — Ana sayfa + cila + yayın:** ana sayfa/hakkında, SEO tamamlama (metadata/sitemap/JSON-LD/OG görseli), a11y turu, bundle analizi, Lighthouse ≥95 hedefi, README (mimari + ekran görüntüleri + canlı link — CV varlığı), Search Console'a sitemap.
